...
PINECONE_INDEX_NAME = "cognitive-research-hub"
PINECONE_ENVIRONMENT = "us-east-1-aws"  # Change based on your region
...
def initialize_rag_system():
    """Initialize Pinecone and Cohere for RAG functionality"""
    global embeddings, pinecone_index, vector_store
    
    try:
        if not COHERE_API_KEY:
            print("⚠️ COHERE_API_KEY not found - RAG disabled")
            return False
            
        if not PINECONE_API_KEY:
            print("⚠️ PINECONE_API_KEY not found - RAG disabled")
            return False
        ...
        if PINECONE_INDEX_NAME not in pc.list_indexes().names():
            ...
            dimension=1024,
            metric="cosine",  # or "dotproduct", depending on your search type
            spec=ServerlessSpec(
                cloud="aws",
                region="us-east-1"
            )
...
async def formatter_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        prompt_template = ChatPromptTemplate.from_messages([
            ...
            If available use the provided document context and previous conversation history to ensure the query is relevant and builds on previous discussions and follow-up questions.
            Mention if user asks an elaborated answer. (Eg. "Explain the concept of X in detail")
            Make it:
            1. Clear and specific
            2. Search-engine friendly
            3. Maximum 10 words with proper keywords
...
async def enhanced_formatter_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        question_to_format = state.get("generated_question") or state["user_question"]
        print("=====----question_to_format----=====", question_to_format)
        prompt_template = ChatPromptTemplate.from_messages([
            ("system", """
...
def store_research_data_in_rag_node(state: ResearchState) -> None:
    """Store all research data from current session into Pinecone for RAG retrieval"""
    if not vector_store:
        print("⚠️ RAG not initialized, skipping storage")
        return
    
    try:
        print("📊 Storing research data in RAG system...")
        
        documents_to_store = []
        session_id = state.get("session_id", "unknown")
        question = state.get("formatted_question", state.get("original_question", ""))
        ...
        if state.get("uploaded_documents_raw"):
            for doc in state["uploaded_documents_raw"]:
                all_sources.append({
                    ...
                    "metadata": {"type": "document", "session_id": session_id}
                })
        
        # Add internet results
        if state.get("internet_results_raw"):
            print("Storing internet search results...")
            for result in state["internet_results_raw"]:
                all_sources.append({
                    "content": result,
                    "source_type": "internet_search",
                    "metadata": {"type": "internet", "session_id": session_id}
                })
        
        # Add arxiv results
        if state.get("arxiv_results_raw"):
            print("Storing ArXiv search results...")
            for paper in state["arxiv_results_raw"]:
                all_sources.append({
                    "content": paper,
                    "source_type": "arxiv_paper",
                    "metadata": {"type": "arxiv", "session_id": session_id}
                })
        
        # Add research summary
        if state.get("research_summary"):
            print("Storing research summary...")
            all_sources.append({
                "content": state["research_summary"],
                "source_type": "research_summary",
                "metadata": {"type": "summary", "session_id": session_id}
            })
        if state.get("source_summaries"):
            print("Storing source summaries...")
        ...
        if documents_to_store:
            ...
            print(f"✅ Stored {len(documents_to_store)} chunks in RAG system")
        else:
            print("⚠️ No documents to store in RAG system")
            
    except Exception as e:
        print(f"❌ RAG storage failed: {e}")
def retrieve_from_rag(query: str, session_id: str, k: int = 10) -> str:
    """Retrieve relevant information from RAG system"""
    if not vector_store:
        print("⚠️ RAG not initialized, returning empty context")
        return ""
    
    try:
        print(f"🔍 RAG retrieval: query='{query[:50]}...', k={k}")
        
        # Retrieve documents
        retriever = vector_store.as_retriever(
            search_kwargs={
                "k": k,
                "filter": {"session_id": session_id}  # Filter by current session
            }
        )
        ...
        for doc in docs:
            ...
            contexts.append(f"[{source_type.upper()}]: {content}")
        
        combined_context = "\n\n---\n\n".join(contexts)
        print(f"✅ RAG retrieved {len(docs)} relevant chunks")
        #print(f"Combined_context: {combined_context}")  # Preview first 200 chars
...
async def report_generation_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        prompt_template = ChatPromptTemplate.from_messages([
    ("system", """You are an expert research analyst generating comprehensive, detailed reports for academic purposes.
     Any irrelevant data or content not related to the user's question or academic research should be excluded in your output.

        CRITICAL INSTRUCTIONS:
        1. Create a comprehensive research report with substantial detail
        2. Use ALL provided content and your knowledge base to write thorough analysis
        3. Structure with clear sections: Summary, Methodology Overview, System Architecture, Key findings, Real world example (This should be explained in layman terms), Conclusion
    ...
    Make sure all the sections are properly elaborated in detail, and the report is structured in a way that it can be used as a standalone document for academic purposes.
...
async def qa_specialist_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        if follow_up_type == "research_followup":
            k_results = 10
            rag_context = retrieve_from_rag(user_question, session_id, k_results)
            if not rag_context:
                print("⚠️ RAG context empty, falling back to source summaries")
                context = combined_context
            else:
                context = rag_context[:4000]  # Limit context size
                print(f"-=-=-=--=RAG retrieval ({k_results} chunks -=-=--=--==-=-)")
            prompt_data = {
            "chat_history": chat_context,
            "context": f"Context from previous sources: {context} \n\n New context from multiple sources: {combined_context}",
...
2. Reference only available, verifiable sources from the accessible knowledge base and given context (If available). If detailed information is explicitly requested — or necessary for clarity — provide thorough, step-by-step explanations with examples where applicable.
...
"""
            ...
            if not rag_context:
                ...
                context = rag_context[:8000]  # Limit context size
                print(f"-=-=-=--=RAG retrieval ({k_results} chunks -=-=--=--==-=-)")
            prompt_data = {
            "chat_history": chat_context,
            "context": context,
...
def route_entry(state: ResearchState) -> Literal["document_processor", "formatter", "enhanced_formatter"]:
    ...
    try:
        ...
        if state.get("uploaded_documents_raw") and len(research_summary) == 0:
            ...
            is_followup = bool(user_question) and len(research_summary) > 0
            print(f"--=-==--===length of research_summary: {len(research_summary)}")
            if len(research_summary) > 0:
...
def create_research_workflow():
    ...
    try:
        ...
        workflow.add_node("Rag_storage", store_research_data_in_rag_node)
        ...
        workflow.add_edge("report_generator", "Rag_storage")
        workflow.add_edge("Rag_storage", END)
...
def create_professional_workflow_diagram():
    """Create a clean, professional workflow diagram in LangGraph style with dynamic node sizing"""
    ...
    # Clean professional colors (similar to LangGraph default)
    colors = {
        'start_end': '#8E7CC3',      # Soft purple for start/end
        'processing': '#A5B4FC',      # Light blue for processing
        'search': '#C084FC',          # Light purple for search
        'generation': '#FBBF24',      # Soft yellow for generation
        'storage': '#FB7185',         # Soft pink for storage
        'decision': '#34D399',        # Soft green for decisions
        'edge': '#6B7280',           # Gray for edges
        'text': '#374151'            # Dark gray for text
    }
    
    def calculate_node_size(text):
        """Dynamically calculate node size based on text length"""
        lines = text.split('\n')
        max_line_length = max(len(line) for line in lines)
        width = max(2.0, max_line_length * 0.15 + 1.0)  # Dynamic width
        height = max(0.6, len(lines) * 0.3 + 0.4)       # Dynamic height
        return width, height
    
    # Node definitions with clear, descriptive names
    nodes_data = {
        'START': {
            'pos': (6, 19), 'text': '__start__', 'color': colors['start_end'], 
            'shape': 'oval', 'category': 'Entry Point'
        },
        'document_processor': {
            'pos': (3, 17), 'text': 'document_processor\n(PDF/DOCX/TXT Processing)', 
            'color': colors['processing'], 'shape': 'rect', 'category': 'Document Processing'
        },
        'query_generator': {
            'pos': (3, 15), 'text': 'query_generator\n(Extract Research Questions)', 
            'color': colors['processing'], 'shape': 'rect', 'category': 'Query Processing'
        },
        'formatter': {
            'pos': (9, 13), 'text': 'formatter\n(Web search Query Enhancement)', 
            'color': colors['processing'], 'shape': 'rect', 'category': 'Basic Processing'
        },
        'enhanced_formatter': {
            'pos': (3, 13), 'text': 'enhanced_formatter\n(Context-Aware Enhancement)', 
            'color': colors['processing'], 'shape': 'rect', 'category': 'Advanced Processing'
        },
        'internet_search': {
            'pos': (6, 11), 'text': 'internet_search\n(Web Search & Analysis)', 
            'color': colors['search'], 'shape': 'rect', 'category': 'Web Research'
        },
        'arxiv_search': {
            'pos': (6, 9), 'text': 'arxiv_search\n(Academic Paper Retrieval)', 
            'color': colors['search'], 'shape': 'rect', 'category': 'Academic Research'
        },
        'handle_insufficient_data': {
            'pos': (2, 7), 'text': 'handle_insufficient_data\n(Data Validation & Recovery)', 
            'color': colors['generation'], 'shape': 'rect', 'category': 'Error Handling'
        },
        'summarize_sources': {
            'pos': (10, 7), 'text': 'summarize_sources\n(Research Synthesis)', 
            'color': colors['generation'], 'shape': 'rect', 'category': 'Content Synthesis'
        },
        'qa_specialist': {
            'pos': (2, 4), 'text': 'qa_specialist\n(RAG-Powered Q&A)', 
            'color': colors['generation'], 'shape': 'rect', 'category': 'Interactive Q&A'
        },
        'report_generator': {
            'pos': (10, 4), 'text': 'report_generator\n(Structured Report Output)', 
            'color': colors['generation'], 'shape': 'rect', 'category': 'Report Creation'
        },
        'Rag_storage': {
            'pos': (6, 1.3), 'text': 'Rag_storage\n(Pinecone Vector Database)', 
            'color': colors['storage'], 'shape': 'rect', 'category': 'Vector Storage'
        },
        'END': {
            'pos': (6, -0.4), 'text': '__end__', 'color': colors['start_end'], 
            'shape': 'oval', 'category': 'Exit Point'
        }
    }
    ...
    for node_id, props in nodes_data.items():
        ...
        width, height = calculate_node_size(text)
        
        if shape == 'oval':
            # Oval shape for start/end nodes
            oval = Ellipse((x, y), width * 1.2, height * 1.5, 
            ...
            rect = FancyBboxPatch(
                ...
                facecolor=color, alpha=0.8,
                edgecolor='#4B5563', linewidth=1.5
            )
            ax.add_patch(rect)
        ...
        # Main text (bold)
        ax.text(x, y + (0.1 if sub_text else 0), main_text, 
                ha='center', va='center',
        ...
        # Sub text (regular)
        if sub_text:
            ax.text(x, y - 0.15, sub_text, 
                    ha='center', va='center',
                    fontsize=9, fontweight='normal', color=colors['text'],
                    fontfamily='Arial', alpha=0.8)
    nodes_data['helper_qa_to_end'] = {
    'pos': (2, -0.4),
    'text': '',  # invisible
    'color': '#FFFFFF',
    'shape': 'rect',
    ...
    # Define edges (both regular and conditional)
    edges = [
        # Regular edges (solid lines)
        ('document_processor', 'query_generator', 'solid'),
        ('enhanced_formatter', 'internet_search', 'solid'),
        ('formatter', 'internet_search', 'solid'),
        ('internet_search', 'arxiv_search', 'solid'),
        # === L-shaped edge: qa_specialist ──→ END (via helper) ===

        ('report_generator', 'Rag_storage', 'solid'),
        ('Rag_storage', 'END', 'solid'),
        
        # Conditional edges (dashed lines)
        ('START', 'document_processor', 'dashed'),
        ('START', 'formatter', 'dashed'),
        ('query_generator', 'enhanced_formatter', 'dashed'),
        ('query_generator', 'formatter', 'dashed'),
        ('arxiv_search', 'handle_insufficient_data', 'dashed'),
        ('arxiv_search', 'summarize_sources', 'dashed'),
        ('handle_insufficient_data', 'qa_specialist', 'dashed'),
        ('handle_insufficient_data', 'report_generator', 'dashed'),
        ('summarize_sources', 'qa_specialist', 'dashed'),
        ('summarize_sources', 'report_generator', 'dashed'),
    ]
    qa_x, qa_y_center = nodes_data['qa_specialist']['pos']
    _, qa_height = calculate_node_size(nodes_data['qa_specialist']['text'])

    # Calculate bottom of qa_specialist
    qa_y = qa_y_center - qa_height / 2 - 0.15  # optional margin
    helper_x, helper_y = 2, -0.4


    # First segment: horizontal line (no arrow)
    line1 = ConnectionPatch(
        (qa_x, qa_y), (helper_x, helper_y), 'data', 'data',
        linestyle='-', color=colors['edge'], linewidth=2, alpha=0.8,
        arrowstyle='-',  # plain line
    )
    ax.add_patch(line1)

    # Second segment: vertical arrow to left side of END node
    end_x, end_y = nodes_data['END']['pos']
    _, end_height = calculate_node_size(nodes_data['END']['text'])

    # Aim arrow to left edge of END
    arrow_to_end = ConnectionPatch(
        (helper_x, helper_y), (end_x - 1.2, end_y), 'data', 'data',
        arrowstyle='->', mutation_scale=20,
        color=colors['edge'], linewidth=2, alpha=0.8
    )
    ax.add_patch(arrow_to_end)
    
    # Draw edges with clean styling
    for start, end, style in edges:
        start_x, start_y = nodes_data[start]['pos']
        end_x, end_y = nodes_data[end]['pos']
        
        # Get node heights to offset arrows so they touch node edge, not center
        _, start_height = calculate_node_size(nodes_data[start]['text'])
        _, end_height = calculate_node_size(nodes_data[end]['text'])
        
        # Determine direction of the arrow (upward or downward)
        margin = max(0.2, start_height * 0.15) # 🔧 adjust this slightly to move arrow outside the node edge

        if start_y > end_y:
            # Arrow goes down
            start_point = (start_x, start_y - start_height / 2 - margin)
            end_point = (end_x, end_y + end_height / 2 + margin)
        else:
            # Arrow goes up
            start_point = (start_x, start_y + start_height / 2 + margin)
            end_point = (end_x, end_y - end_height / 2 - margin)
        ...
        alpha = 0.7 if style == 'dashed' else 0.8
        
        arrow = ConnectionPatch(
            start_point, end_point, 'data', 'data',
            arrowstyle='->', mutation_scale=20,
            color=colors['edge'], linewidth=2, alpha=alpha,
            linestyle=linestyle
        )
        ax.add_patch(arrow)
    ...
    ax.text(6, 20.6, 'Cognitive Research Hub', ha='center', va='center',
            fontsize=20, fontweight='bold', color='#1F2937',
            fontfamily='Arial')
    ax.text(6, 20.2, 'Multi-Agent Research Workflow', ha='center', va='center',
            fontsize=14, color='#6B7280', fontfamily='Arial')
    # Spacer (invisible text to reserve vertical space)

    # Simple legend
    legend_elements = [
        mpatches.Patch(color=colors['start_end'], label='Start/End Points', alpha=0.8),
        mpatches.Patch(color=colors['processing'], label='Processing Nodes', alpha=0.8),
        mpatches.Patch(color=colors['search'], label='Search Operations', alpha=0.8),
        mpatches.Patch(color=colors['generation'], label='Generation & Analysis', alpha=0.8),
        mpatches.Patch(color=colors['storage'], label='Storage Operations', alpha=0.8),
        plt.Line2D([0], [0], color=colors['edge'], linewidth=2, label='Sequential Flow'),
        plt.Line2D([0], [0], color=colors['edge'], linewidth=2, linestyle='--', label='Conditional Flow')
    ]
    
    ax.legend(handles=legend_elements, loc='upper right', bbox_to_anchor=(0.98, 0.95),
              title='Component Types', title_fontsize=12, fontsize=10,
              framealpha=0.9, edgecolor='#E5E7EB')
    

    
    plt.tight_layout()
    plt.savefig("clean_professional_workflow.png", dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.show()


create_professional_workflow_diagram()
...
async def startup_event():
    ...
    if not initialize_rag_system():
        print("⚠️ RAG system initialization failed. QA will use fallback method.")
...
async def generate_mock_test(session_id: str):
    ...
    # 1. Add the main research query
    if state.get("original_question"):
        context_parts.append(f"Main Research Query: {state['original_question']}")
    
    # 2. Add research summary (most important)
    if state.get("research_summary"):
        context_parts.append(f"Research Summary:\n{state['research_summary']}")
    
    # 3. Add uploaded documents
    if state.get("uploaded_documents_raw"):
        context_parts.extend(state["uploaded_documents_raw"])
    
    # 6. Add chat history content (FIXED - extract actual content)
    if state.get("chat_history"):
        chat_content = []
        for msg in state["chat_history"]:
            if msg.get("role") == "user":
                chat_content.append(f"User Question: {msg.get('content', '')}")
            elif msg.get("role") == "assistant":
                chat_content.append(f"Assistant Response: {msg.get('content', '')}")
...
async def submit_mock_test(session_id: str, payload: AnswerPayload):
    ...
    The LLM receives:
      ...
      • All research material collected during the session (excluding internet/arxiv).
    ...
    # ── Build comprehensive evidence packet (SAME AS QUIZ GENERATION) ─────────────────────────────────────
    context_parts = []
    ...
    # 3. Add uploaded documents ONLY
    if state.get("uploaded_documents_raw"):
        context_parts.extend(state["uploaded_documents_raw"])
    
    # 4. Add chat history content (extract actual content)
    if state.get("chat_history"):
        chat_content = []
        for msg in state["chat_history"]:
            if msg.get("role") == "user":
    ...
    if not context_parts:
        raise HTTPException(status_code=400,
                            detail="No background material available; cannot evaluate answers.")

    # Combine all context with clear separators
    evidence_blob = "\n\n---SECTION BREAK---\n\n".join(context_parts)[:8_000]
