...
def safe_extract_text_from_file(file_path: str) -> str:
    ...
    try:
        ...
        print(f"❌ File extraction failed for {file_path}: {e}")
        return ""
...
async def formatter_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        prompt_template = ChatPromptTemplate.from_messages([
            ...
            a well-structured research query suitable for academic and web searches. 
        ...
        chain = prompt_template | fast_llm
        prompt_data = {"question": question_to_format}
...
async def qa_specialist_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        prompt_template = ChatPromptTemplate.from_messages([
            ("system", """You are an expert research assistant with comprehensive access to:
...
CAPABILITIES:
✅ Answer questions using all available knowledge sources
✅ Reference specific sources and documents when making claims
✅ Maintain conversation context from chat history
✅ Provide detailed, well-formatted responses
✅ Handle follow-up questions that build on previous exchanges

INSTRUCTIONS:
1. Use the conversation history to understand context and follow-up questions
2. Reference the comprehensive knowledge base to provide accurate and concise information (If explicitly asked for elaborated or detailed information, provide it)
3. If information isn't available in the sources, clearly state this limitation
4. Format responses in clean HTML for better readability
5. Be thorough and helpful while remaining accurate
...
Remember: You have access to the complete research context from this session.
"""),
...
def create_research_workflow():
    ...
    try:
        ...
        workflow.add_edge("query_generator", "formatter")
        ...
        workflow.add_edge("summarize_sources", "report_generator")
        workflow.add_edge("handle_insufficient_data", "report_generator")
...
def create_enhanced_research_workflow():
    """Create workflow with enhanced QA capabilities"""
    try:
        workflow = StateGraph(ResearchState)
        ...
        # Add enhanced QA node
        workflow.add_node("enhanced_qa", qa_specialist_node)
        
        # Keep existing workflow connections
        workflow.add_conditional_edges(
            START,
            route_entry,
            {
        ...
        research_app = workflow.compile()
        print("✅ Enhanced research workflow compiled successfully")
        return research_app
...
def visualize_langgraph_workflows():
    ...
    try:
        ...
        enhanced_workflow = create_enhanced_research_workflow()
        
        workflows = [
            ("Research Workflow", research_workflow),
            ("Enhanced Research Workflow", enhanced_workflow)
        ]
...
3. Questions must be derived ONLY from the provided context
4. The order of questions should be from easy to hard (basic to advanced)
...
OUTPUT REQUIREMENTS - Return ONLY in this structure:
    ...
    AI Study Assistant Prompt:
        - The prompt must instruct an AI tutor to help the student master their weak topics using detailed explanations, examples, and practice.
